...
public class Label extends ObjectWithId implements Comparable<Label> {
	 ...
	 * the id can change)
	 */
	public static final Map<Integer, Integer> iconsMap;
	...
	static {
		...
		iconsMapInv = new HashMap<Integer, Integer>(iconsMap.size());
		iconsList = new ArrayList<Integer>(iconsMap.size());
		for (Entry<Integer, Integer> entry : iconsMap.entrySet()) {
			iconsMapInv.put(entry.getValue(), entry.getKey());
			iconsList.add(entry.getValue());
		}
	}

	public static List<Integer> getIconsList() {
		return iconsList;
	}
